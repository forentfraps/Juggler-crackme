...
extern void _MixColumn(unsigned char *operand, void *table);
extern void _invMixColumn(unsigned char *operand, void *table);
...
void KeyScheduler(unsigned char *MasterKey, unsigned char *dest) {
  /*
  MasterKey is 16 bytes long, dest should be 176 bytes long, since it has 11
  keys in it
  */
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    XorI32(G_storage, &(tmp[0 * sizeof(unsigned long)]),
           &(scratch[0 * sizeof(unsigned long)]));
    XorI32(&(scratch[0 * sizeof(unsigned long)]),
           &(tmp[1 * sizeof(unsigned long)]),
           &(scratch[1 * sizeof(unsigned long)]));
    XorI32(&(scratch[1 * sizeof(unsigned long)]),
           &(tmp[2 * sizeof(unsigned long)]),
           &(scratch[2 * sizeof(unsigned long)]));
    XorI32(&(scratch[2 * sizeof(unsigned long)]),
           &(tmp[3 * sizeof(unsigned long)]),
           &(scratch[3 * sizeof(unsigned long)]));
