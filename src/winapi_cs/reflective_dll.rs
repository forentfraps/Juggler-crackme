...
pub unsafe fn ReflectiveLoadDll(dllBytes: *mut BYTE) -> Option<*mut BYTE> {
    ...
    while (*importDescriptor).Name != 0 {
        ...
        if !library.is_null() {
            let mut thunk =
                dllBase.offset((*importDescriptor).FirstThunk as isize) as *mut IMAGE_THUNK_DATA;
            while (*thunk).Ordinal != 0 {
                let functionAddress = if IMAGE_SNAP_BY_ORDINAL64((*thunk).Ordinal) {
                    GetProcAddress(library, IMAGE_ORDINAL64((*thunk).Ordinal) as LPCSTR)
                } else {
                    let importByName = (dllBase as usize + (*thunk).Ordinal as usize)
                        as *const IMAGE_IMPORT_BY_NAME;

                    GetProcAddress(library, (*importByName).Name.as_ptr() as *const i8)
                };

                (*thunk).Ordinal = functionAddress as u64;
